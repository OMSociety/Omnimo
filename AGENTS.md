...
| 11 | Gallery 的面板图标是**固定位置的图层**（`@Resources\Graphics\Gallery\mask-*.png`，由 `[Cat1Mask]` 之类的表绘制） | 在满行中间插一格磁贴，会把它后面所有格的图标顶错位；删掉一格，其后标签会顶到前一格的图标上 | 增删磁贴要**同时改两处**：`catN.inc` 里的磁贴表 + 同一位置那张图里的字形。图标层几何实测：`mask-essential.png` 是 2 倍图，列中心 `57,178,300,421,543,664,786,907`（格距 121.43 图内像素 = 60.92 逻辑像素，等于磁贴间距 61），格子 = 中心 ±60.7；行中心按内容带实测（如时间与日期第 1/2 行 = 391/513，信息第 1 行 = 717.5）。改字形请**复制它自己的字形**（如日历取 Date 那格），移动一段时把后续字形整体挪一格并清空末尾格。另外：不要用叠加表去盖图标——相对定位（`Y=...R`/`X=...r`）只看文件里上一个表，把辅助表夹在磁贴之间会把其后所有区块的标签整体带偏（实测）；确需叠加时必须用 `[表名:X]`/`[表名:Y]` 定位并放在文件末尾 |
